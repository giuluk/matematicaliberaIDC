...
\node (d) at (-4.5, 4.5) {dividendo};
\node (r) at (-3, .8) {resto};
\node (di) at (4.5, 4.5) {divisore};
\node (q) at (5, 3.2) {quoziente};
\end{scope}

\begin{scope}[->]
\draw (d.east)--(a-1-2.west);
\draw (r.east)--(a-5-4.west);
\draw (di.west)--(a-1-5.east);
\draw (q.west)--(a-2-6.east);
\end{scope}
\end{tikzpicture}
}
