...
\item mcm e MCD fra polinomi;
\item le frazioni algebriche rappresentano la divisione fra due polinomi;
\item in alcuni casi una frazione algebrica è un'espressione senza significato;
\item le operazioni fra le frazioni algebriche seguono le regole apprese
nel calcolo con le frazioni numeriche.
\end{itemize}
}
...
Così, proprio come abbiamo fatto con i numeri interi, anche con i 
polinomi 
...
prima di tutto scomporli in fattori irriducibili\indc{polinomio/polinomi}
{fattori irriducibili di un polinomio}.
...
prudentemente si dovrebbe parlare di divisore comune e di multiplo 
comune.
\end{osservazione}

Un polinomio~\(A\) si dice multiplo di un 
polinomio~\(B\)\indc{polinomio/polinomi}{polinomio multiplo di polinomio} 
se esiste un polinomio~\(C\) per il quale~\(A=B\cdot C\) in questo caso 
diremo anche che~\(B\) è divisore del polinomio~\(A\).

\subsection{Massimo Comun Divisore}
\indc{polinomio/polinomi}{MCD di polinomi}
...
% \begin{definizione}{}{}
% Dati due o più polinomi, il massimo comune divisore è il più grande 
% polinomio che è divisore di tutti.
% \end{definizione}
% 
% \begin{osservazione}{}{}
% Con ``più grande'' (o ``più piccolo'') si intende ``di grado maggiore'' 
% (o ``di grado minore'').
% \end{osservazione}
...
comuni ai polinomi, presi ciascuno una sola volta, con il minimo 
esponente.
\end{teorema}

% Sia i coefficienti numerici, sia i monomi possono essere considerati 
% polinomi.
...
\item \(12a^{2}b^{2}-24ab^{2}+12b^{2}=12b^{2} \tonda{a^{2}-2a+1} =
        12b^{2}(a-1)^{2}\).
\end{itemize}
\item i fattori comuni a tutti i polinomi presi con l'esponente più 
piccolo sono: \\
...
\indc{polinomio/polinomi}{mcm di polinomi}
...
\begin{teorema}{}{}
Dopo aver scomposto ciascun polinomio in fattori, il minimo comune 
multiplo tra due o più polinomi è il prodotto dei fattori comuni e non 
comuni di tutti i polinomi, presi una sola volta, con il massimo 
esponente.
\end{teorema}
...
non cambiano. 
Infatti sia i numeri che i monomi possono essere visti come particolari 
polinomi.
\end{osservazione}
...
due polinomi è un polinomio, altre volte la divisione tra polinomi dà 
un resto.
In questo caso il quoziente esatto dei due polinomi non è un polinomio, 
ma una \indt{frazione algebrica}, cioè una frazione che ha un polinomio 
al numeratore e uno al denominatore.
...
\affiancati{.58}{.38}{
Quando l'espressione contiene una sola variabile, può essere vista come 
una funzione ``reale'' cioè una funzione che associa ad ogni numero 
reale al massimo un altro numero reale.
...
\immagine[.8]{Grafico della precedente frazione algebrica}
{\graficofrazionea}
}
...
% espresso da una \textbf{frazione algebrica}.
...
Poiché non è mai possibile dividere per~\(0\), una frazione algebrica 
perde di significato per quei valori che attribuiti alle variabili 
rendono il denominatore uguale a zero. 
...
\indtc{f.algebrica}{condizione di esistenza} (\(\CE\)):~\(B\neq~0\).

La funzione precedente non dà quindi un risultato quando: 
\(2x^2 +4x -16=0\).
...
\central{\(\graffa{\forall x \in \R | x \ne -4 \sand x \ne +2}\)}

\begin{esempio}{}{}
Determinare le condizioni di esistenza di esistenza delle seguenti 
funzioni, cioè studiare per quali valori delle variabili il denominatore 
è diverso da zero.
...
Nell'insieme dei numeri reali, il prodotto di più fattori è nullo se 
almeno uno dei fattori è nullo\ind{annullamento del prodotto}.
\end{teorema}

Quando è possibile, dobbiamo scomporre in fattori il denominatore e 
studiare ogni singolo fattore ponendolo diverso da zero.
...
La condizione \(x \neq -2 \stext{e} x \neq +2\) può anche essere 
sintetizzata con l'espressione: \(x \neq \mp 2\).
\end{esempio}
% \end{exrig}
...
% Nel proporre i prossimi esempi ed esercizi daremo per scontata la 
% condizione di esistenza delle frazioni proposte, a meno che non diventi 
% necessario discuterne.
% \ovalbox{\risolvii \ref{ese:19.1}, \ref{ese:19.2}, \ref{ese:19.3}, 
% \ref{ese:19.4}}

\section{Semplificazione di una frazione 
algebrica}\indc{f.algebriche}{semplificazione}
\label{sec:frazalg_semplificazione}

Semplificare una frazione algebrica significa dividere numeratore e 
denominatore per uno stesso fattore diverso da zero, in questo modo 
infatti la proprietà invariantiva della divisione garantisce che la 
frazione non cambia di valore.
...
È chiaro che quest'ultimo calcolo non è accettabile: infatti 
semplificare una frazione significa ottenerne un'altra con numeri più 
piccoli, ma equivalente alla prima e \(\dfrac{4}{7}\) è equivalente a 
\(\dfrac{8}{14}\) mentre \(\dfrac{2}{3}\) non lo è.
...
La semplificazione di una funzione razionale produce un'altra funzione 
che si comporta \emph{quasi} sempre come la funzione di partenza.

\affiancati{.48}{.48}{
\immagine[.8]{Grafico di ((x-3)(x-1))/((x-3)(x+2))}{\graficofrazioneb}
}{
\immagine[.8]{Grafico di (x-1)/(x+2)}{\graficofrazionec}
}

Le funzioni \(f_1\) e \(f_2\) sono uguali dappertutto tranne che in 
\(x=+3\) dove \(f_1\) non è definita mentre \(f_2\) vale 
\(\dfrac{2}{5}\).
% : 
...
\begin{esempio}{}{}
Ridurre ai minimi termini la frazione:\quad 
\(\dfrac{a^{2}-6a+9}{a^{4}-81}\).
\begin{enumerate} [left=0mm]
\item Scomponiamo in fattori
\begin{itemize}[left=0mm, nosep]
\item il numeratore:~\(a^2 - 6a +9 = (a - 3 )^2\)
\item il denominatore:
\(a^4 - 81 = \tonda{a^2 - 9} \tonda{a^2 + 9} = 
...
\[\dfrac{a^{2}-6a+9}{a^{4}-81} \stackrel{1}{=} 
  ...
  a \neq +3\]
Dove nel passaggio 1 si scompone in fattori, nel passaggio 2 si 
semplifica, nel passaggio 3 si dichiara per quali valori la funzione 
ottenuta è effettivamente uguale a quella di partenza.
\end{esempio}
...
Il prodotto di due frazioni\indc{f.algebriche}{prodotto tra f.algebriche} 
...
\(f_{2}=\dfrac{25bc^{7}}{9}\)}\\ [-.5em]

Il prodotto si ottiene semplificando e ponendo le condizioni per ogni 
fattore semplificato: 
...
\\ [-.5em]
In questo caso non è possibile alcuna semplificazione, il prodotto si 
ottiene moltiplicando fra loro i numeratori e i denominatori:
...
\central{
\(f_{1}=\dfrac{-x+2x^2}{x^2-3x+2}\) ~~e~~ 
\(f_{2}=\dfrac{5x-5}{x-4x^2+4x^3}\)}\\ [-.5em]
...
\vspace{-2.0em}
\[f_1 \cdot f_2 = 
\dfrac{\cancel{x} \cdot \cancel{(2x-1)}}{\cancel{(x-1)} \cdot {(x-2)}} 
\cdot \dfrac{5\cdot\cancel{(x-1)}}{\cancel{x}\cdot(2x-1)^{\cancel{2}}}=
\dfrac{5}{(x-2)(2x-1)} \text{ con }
\sistema{x \neq 0 \\ x \neq +1 \\ x \neq +\dfrac{1}{2}}
\]
% {\central
% \(\text{con } x \neq 0 \stext{e} x \neq +1 \stext{e} x \neq +\dfrac{1}{2}\)}
%  \item esplicitiamo le~\(\CE\) relative ai fattori che sono stati 
%semplificati:
% \[\CE \sistema{x \neq 0 \\
%       x-1 \neq 0 \\
%       2x-1 \neq 0}
...
divisione in una moltiplicazione quindi si può sempre eseguire la 
divisione, a patto che il divisore sia diverso da 
zero\indc{f.algebriche}{divisione di f.algebriche}.
L'insieme delle frazioni algebriche è chiuso anche per la divisione.

Il quoziente di due frazioni è la frazione che si ottiene moltiplicando 
la prima per il reciproco della seconda.
Lo schema di calcolo può essere illustrato nel modo seguente, come del 
resto abbiamo visto nell'insieme dei numeri razionali:
...
\central{
\(f_{1}=\dfrac{3a-3b}{2a^{2}}\) \quad e \quad 
\(f_{2}=\dfrac{a^{2}-ab}{b^{2}}\)}%\\ [-.5em]

\begin{enumerate} [left=0mm]
\item Esplicitiamo la condizione di esistenza del divisore: \(b \neq 0\)
\item Scomponiamo in fattori i polinomi\\
\begin{itemize*}[nosep]
\item \(f_{1}=\dfrac{3a-3b}{2a^{2}}=\dfrac{3\cdot(a-b)}{2a^{2}}\qquad\)
...
\indc{f.algebriche}{potenza di una f.algebrica}
...
Se~\(n = 0\) sappiamo che qualsiasi numero \emph{diverso da zero}, 
...
Se~\(n\) è intero negativo la potenza con base diversa da zero è uguale 
alla potenza che ha per base il reciproco della base e per esponente 
l'opposto dell'esponente. 
\( \tonda{\dfrac{A}{B}} ^{-n}=
\tonda{\dfrac{B}{A}} ^{+n} \sstext{con} A\neq~0\) e~\(B\neq~0\).

Quindi deve esistere sia la frazione di partenza, sia il suo reciproco, 
cioè dovranno essere diversi da zero sia il denominatore sia il 
numeratore.
...
\section{Addizione di frazioni algebriche}\indc{f.algebriche}{somma tra 
f.algebriche}
...
Come per le frazioni numeriche, anche nelle frazioni algebriche 
l'addizione è l'operazione più complicata. Infatti per addizionare due 
frazioni algebriche bisogna:
...
\item semplificare il risultato ottenuto ponendo le condizioni di 
esistenza.
\end{enumerate}
\end{procedura}
...
\item \emph{addizionare i numeratori} eseguiamo le operazioni al 
numeratore:
...
%  quoziente ottenuto per il relativo numeratore:
%  
% \[\dfrac{2y^{2}\cdot(x+y)}{6x^{2}y^{3}}-
% \dfrac{3x\cdot(2y-x)}{6x^{2}y^{3}}\]
...
% 
% 
% \dfrac{2x-3y}{x+y}+\dfrac{x+2y}{x+y}=\dfrac{(2x-3y)+(x+2y)}{x+y}=
% \dfrac{3x-y}{x+y}.
% \end{equation*}
%  \end{esempio}
%  
% \begin{osservazione}{}{} 
% A questo caso ci si può sempre ricondurre trasformando le 
