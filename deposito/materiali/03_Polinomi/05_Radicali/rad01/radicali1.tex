...
\input{\folder radicali1_grafici}

\chapter{Radicali}

\inicapitolo{
\begin{itemize}[left=0mm, nosep]
\item  potenza e radice sono operazioni inverse l'una dell'altra;
\item un'infinità di radici non ha risultato reale, ma di altre radici
consideriamo solo uno dei risultati possibili;
\item \(\sqrt{2}\) è un numero speciale per più di un motivo;
\item operazioni e espressioni con i radicali;
\item i numeri irrazionali e i numeri reali;
\item la retta cartesiana è un buon modello per l'insieme dei reali;
\item il valore assoluto, le sue proprietà, le funzioni associate.
\end{itemize}
}
...
delle operazioni inverse. Un'\indt{operazione inversa} rispetto a un'altra 
...
% Ad esempio se a un numero qualunque (per esempio \(142\)) aggiungo~7 
% (\(+7\), operazione diretta) l'effetto ottenuto è il numero \(149\). Ma se 
% poi tolgo~7 (\(-7\), operazione inversa) ottengo ancora il numero di 
% partenza. 
% Possiamo dire che la sottrazione è l'operazione inversa 
% dell'addizione. Così pure, se parto da un numero~\(a\) e lo moltiplico 
% per~\(b\) poi lo divido per~\(b\) (supponiamo \(b\neq 0\)), ottengo ancora il 
% numero~\(a\):
% 
% \(142 + 7 - 7 = 149 -7 = 142\)
% 
...
% \(5 + \dots = 9 => \dots = 9 - 5\)
% 
% \(\dots \times 3 = 6 => \dots = 6 : 3\)
% 
% \(15 \times \dots = 5 => \dots = 5 : 15\)
% % \end{htmulticols}
% 
...
% \(5 - \dots = 9 => 5 =  9\overset{?}{\dots}\)
% 
% \(\dots : 3 = 6 => \dots = 6 \overset{?}{\dots} 3\)
% 
% %\(15 : \dots = 5 => \dots = 15 \overset{?}{\dots} 5\)
% \(15 : \dots = 5 => 15 = 5\overset{?}{\dots} \)
% % \end{htmulticols}
% 
% Perché sottrazione e divisione si comportano in modo diverso da addizione 
% e moltiplicazione? Come si comporterà la potenza?

La potenza ha due operazioni inverse. 
\begin{itemize}
\item 
...
\begin{definizione}{}{}
La \textbf{radice}\ind{radice} è l'operazione che permette di calcolare la base
conoscendo la potenza e l'esponente.
\end{definizione}

\begin{definizione}{}{}
Il \textbf{logaritmo}\ind{logaritmo} è l'operazione che permette di calcolare
...
l'operazione e il risultato dell'operazione: quindi si deve capire dal
contesto il significato della parola ``radice''.
...
quarta dia come risultato~\(-16\):\quad \(x^4=-16\)
...
reale elevato alla quarta può dare un risultato negativo\indc{radici}{C.E. 
delle radici}.
...
\begin{itemize} [nosep]
  \item se il \textbf{radicando~\(a\) è negativo} la \textbf{radice} 
          \textbf{Non è Reale};\indc{radici}{C.E. delle radici}
...
Dalla definizione \(\sqrt{a^2}=a\) con \(a\geqslant 0\), quindi: 
...
\item \(\sqrt{\dfrac 9{16}}=\dfrac 3 4\) infatti \(\left(\dfrac 3 
4\right)^2=
\dfrac 9{16}\)
...
Osserva che la radice cubica di un numero mantiene sempre lo stesso segno 
del numero in quanto il cubo di un numero reale conserva sempre il segno 
della base.
...
di~\(a\geqslant 0\) e si ha che \(\sqrt[n]a \geqslant 0\).
\end{itemize}
...
Quindi \(sqrt{a}\) è un numero reale solo se~\(a \geqslant 0\).
...
di~\(x\) che rendono non negativo il radicando, cioè 
\(\CE: A(x)\geqslant 0\)
...
\begin{itemize}
\item \(\sqrt x\):\quad \(\CE x\geqslant 0\)
\item \(\sqrt[3]x\):\quad \(\CE \forall x\in R\)
\item \(\sqrt{-x}\):\quad \(\CE x\leqslant 0\)
\item \(\sqrt[3]{-x}\):\quad \(\CE \forall x\in R\)
\item \(\sqrt{x-1}\):\quad \(\CE x-1\geqslant 0 \Rightarrow x\geqslant 1\)
...
la 
  ...
  quindi \(\CE x+1\neq 0 \Rightarrow x\neq -1\)
\item \(\sqrt[4]{xy}\):\quad \(\CE xy\geqslant 0\)
...
\affiancati{.49}{.49}{
\immagine{Quadrato con diagonale posto su un asse x.}{\radicediduea}
}{
\immagine{Diagonale del quadrato riportata sull'asse x.}{\radicedidueb}
}
...
% \begin{itemize}
%  \item 
% Supponiamo che la radice di \(2\) sia un numero razionale scritto sotto 
% forma di frazione~\(\sqrt{2}= \frac{a}{b}\), certamente possiamo ridurre 
% la frazione ai minimi termini.
%  \item 
...
Tornando al problema di calcolare la \(\sqrt{2}\) possiamo dire 
che~\(1<\sqrt{2}<2\). 
...
% \begin{osservazione}{}{}
% Da notare che tra le due classi di approssimazione possiamo pensare che ci 
% sia più di un numero, in questo caso ognuno di questi numeri può essere 
% usato come rappresentante della radice di \(2\).
% \end{osservazione}
...
L'insieme dei numeri irrazionali non è chiuso per la moltiplicazione 
...
% \,067\,982\,148\,086\,513\,282\,306\,
% 647\,093\,844\,609\,550\,582\,231\,725\,359\,408\,128\,481\,117\,
% 450\,284\,102\,701\,938\,521\,105\,559\,644\,622\,948\,954\,930\,381\,
% 964\,428\,810\,975\,665\,933\,446\,128\,475\,648\,233\,786\,783\,165\,
% 271\,201\,909\,145\,648\,566\,923\,460\,348\,610\,454\,326\,648\,213\,
% 393\,607\,260 
\ldots 
\end{itemize}
...
\(B=\left\{x\in \Q |\, x \geqslant 3\right\}\) 
...
\item siano \(A=\left\{x\in \Q |\, x \leqslant \frac{2}{7}\right\}\), 
\(B=\left\{x 
\in Q |\, x \geqslant \frac{2}{7}\right\}\), anche in questo caso la coppia 
\((A,B)\) 
non 
...
\begin{postulato}{di continuità della retta}{}\ind{continuità della retta}
...
\section{Valore assoluto}\ind{valore assoluto}
...
% a+1-3a+1=-2a+2 \text{, se }a+1\geqslant~0\Rightarrow a\geqslant-1\\
...
Funzione Associata):\\%[-.5em]
\immagine*[.9]{}{\segnoretta{\small a +1 = 0 \sRarrow a = -1}
           {\small y = a +1 \srarrow}
           {\segnicre[a]{-1}}} \\[-1em]
...
\immagine*[.9]{}{\segnoretta{\small x -5 = 0 \sRarrow x = +5}
           {\small y = x -5 \srarrow}
           {\segnicre[x]{+5}}}\\[-1em]
\immagine*[.9]{}{\segnoretta{\small x +2 = 0 \sRarrow x = -2}
           {\small y = x +2 \srarrow}
           {\segnicre[x]{-2}}} \\[-2.5em]
\immagine{Riunione dei segni precedenti.}{\riunionesegni} 
~\\[-5em]
...
% \(x\geqslant 5\) e l'argomento del secondo valore assoluto è non negativo
% per \(x\geqslant -2\), possiamo porre la reciproca situazione nel seguente 
% grafico:
% % \begin{center}
...
% -2x+3 \text{, se }x<-2\\
% 7 \text{, se }-2\leqslant x<5\\
% 2x-3 \text{, se }x\geqslant 5\end{array}.\right.
% \]
\end{esempio}
...
\central{\(\tonda{b^\frac{1}{n}}^n=b^{\frac{1}{n} \cdot n} = b\) \quad in 
generale: \quad 
...
la base deve essere diversa da zero\indc{radici}{C.E. delle radici}.
\item Dato che abbiamo visto che se l'indice è pari il radicando deve 
essere\indc{radici}{C.E. delle radici}
...
\ind{potenza a esponente razionale} 
...
numero 
reale \(a \geqslant 0\) sotto forma di potenza di \(a\), vogliamo cioè 
che sia: \(\sqrt[n]a=a^x\).
...
\(a=a^{n\cdot x}\).
Trattandosi di due potenze con base~\(a{\geqslant}0\), l'uguaglianza è 
resa possibile solo se sono uguali gli esponenti. 
In altre parole, deve essere: \(1=n\cdot x \Rightarrow x=\dfrac 1 n\), 
quindi: \(\sqrt[n]a=a^{\frac 1 n}\).

Vediamo ora di generalizzare la formula. Sia \(m\) un numero intero 
positivo, possiamo scrivere \(a^{\frac m n}=\left(a^{\frac 1 n}\right)^m\) 
e quindi \(a^{\frac m n}=\left(\sqrt[n]a\right)^m\).
...
radicando per uno stesso numero non nullo.\ind{semplificare radicali} 
In simboli: 
...
% comune. 
% In simboli \(\sqrt[nt]{a^{mt}}=\sqrt[n]{a^m}\) con \(a\geqslant 0\) e 
% \(m,n,t\in \N-\{0\}\).
% \end{teorema}
...
Se il radicando\ind{segno del radicando} ha base negativa e nella 
...
modulo per garantire la concordanza del segno\indc{radici}{modulo del 
radicando}:
...
Il prodotto\indc{radici}{prodotto tra radici} di due radici che hanno lo stesso 
...
Analogamente per il quoziente:\indc{radici}{quoziente di radici} 
...
l'espressione di partenza non ha\indc{radici}{C.E. delle radici}.
...
\subsection{Potenza di radice e radice di radice}
\indc{radici}{potenza/radice di radice}
...
La radice di un'altra radice è uguale a una radice con lo stesso 
radicando che ha per indice il prodotto degli indici delle radici: 
...
\begin{esempio}{}{}
Data l'espressione \(E=\sqrt[5]{3\cdot\sqrt{2}}\) ridurla a unico 
radicale.

In questo caso non possiamo subito applicare la regola annunciata, ma 
dobbiamo  portare il fattore esterno dentro la radice più interna 
ottenendo \(\sqrt[5]{\sqrt{3^2 \cdot 2}}=\sqrt[10]{18}\).

Osserviamo che l'espressione \(E=\sqrt[5]{3+\sqrt{2}}\) non si può 
ridurre ad unico radicale, se non sotto determinate condizioni.
...
\subsection{Somma di radicali}\indc{radici}{somma di radicali}
% \label{sec:radicali_somma}

Si dice \emph{radicale} un'espressione del tipo \(a\sqrt[n]b\) con 
\(a\) e \(b\) numeri reali, \(b \geqslant 0\) ed \(n\in \N\). 
Il numero \(a\) prende il nome di \emph{coefficiente} del radicale.

Operare con i radicali è simile al modo di operare con i monomi. 
Infatti è possibile effettuare somme algebriche soltanto se i radicali 
hanno lo stesso indice e lo stesso radicando, mentre si possono sempre 
effettuare moltiplicazioni e divisioni dopo averli ridotti allo stesso 
indice.
\begin{definizione}{}{}
Due radicali si dicono \textbf{simili} se hanno lo stesso indice e lo 
stesso radicando.
\end{definizione}

È possibile effettuare somme algebriche soltanto se i radicali sono 
simili, si eseguono le somme allo stesso modo in cui si eseguono le 
somme algebriche dei monomi.

Attenzione l'operazione \(\sqrt{2}+\sqrt{3}=\sqrt 5\)\, è errata in 
quanto i radicali addendi non sono simili.
...
\label{sec:radicali_razionalizzazione}\indc{radici}{razionalizzazione}
...
% \((a+b)(a^2-ab+b^2)=a^3+b^3\) e quello analogo~\((a-b)(a^2+ab+b^2)=a^3-
% b^3\).
% \begin{align*}
...
\paragraph{Sistemi di primo grado}~
...
\affiancati{.49}{.49}{
\immagine{Sistema di disequazioni rappresentato su assi cartesiani.}
% {\input{\folder lbr//fig001_is.pgf}}
{\condradicalia}
}{
\vspace{8mm}
In definitiva \(\CE x \geqslant +1\).
}
\end{esempio}
...
\vspace{8mm}
In definitiva C.E. \(x<-1\vee x \geqslant 1\).
}
\end{esempio}
...
Nella semplificazione di radici dobbiamo prestare una certa attenzione.
\begin{esempio}{}{}
...
% con resto uguale a \(2\) per \(b^7\) si ha \(7:3\) con quoziente~\(2\) e 
% resto~\(1\) 
...
% \item \({\small \sqrt[4]{4x^4-4x^5}=\sqrt[4]{4x^4(1-x)}=
