...
\item le disuguaglianze;
\item gli intervalli;
\item il segno di un polinomio lineare;
\item il segno di un prodotto e di un quoziente;
\item le disequazioni numeriche;
\item i sistemi di disequazioni;
\item le disequazioni letterali;
\item alcuni problemi.
\end{itemize}
}
...
\item \(x+\dfrac{1}{2}x\le~1\).
 \end{enumeratea}
\end{htmulticols}
\ind{formule chiuse e formule aperte}
...
\ind{primo membro, secondo membro}
 \begin{esempio}{}{}
...
\ind{estremi di un intervallo}
\vspace{-2mm}
\immagine
{Asse cartesiano con evidenziato un pezzo delimitato da due punti pieni.}
...
\end{itemize}
\ind{intervallo chiuso o aperto}
Possiamo distinguere gli intervalli anche in base a un'altra 
caratteristica:
...
di un dato numero.
\end{itemize}
\ind{intervallo limitato o illimitato}
...
\centering\begin{tabular}{>{\centering\arraybackslash}m{28mm}|
...
\(a \leqslant x \leqslant b\) & \([a;~b]\) &  
\disegno{\inticonasse{0}{5}{1}{4}{a}{b}{blue}{blue}{x}} \\
\hline
i numeri compresi tra \(a\) e~\(b\), a~incl., b~escl. & 
\(a \leqslant x < b\) & \([a;~b)\) o \([a;~b[\) &  
\disegno{\inticonasse{0}{5}{1}{4}{a}{b}{blue}{white}{x}} \\
\hline
i numeri compresi tra \(a\) e~\(b\), a~escl., b~incl. & 
\(a < x \leqslant b\) & \((a;~b]\) o \(]a;~b]\) &  
...
\(x \leqslant a\) & \((-\infty;~a]\) o \(]-\infty;~a]\) &  
...
\(x \geqslant a\) o \(a \leqslant x\) & \([a;~-\infty)\) o \([a;~-\infty[\) & 
...
\begin{esempio}{}{}
\(\P=\{x \in \R/x \geqslant -5\}\) intervallo illimitato superiormente 
chiuso a sinistra~\(\P = [-5;~+\infty[ = [-5;~+\infty)\).
...
\begin{esempio}{}{}
\(S=\{x\in \R/-2 \leqslant x \leqslant 6\}\) intervallo chiuso e 
limitato~\(S = [2;~6]\).
...
% In questo modo, le disequazioni si ridurranno a una applicazione dello 
% studio del segno.
Studiare il segno di un'espressione che contiene la variabile~\(x\), vuol 
dire stabilire per quali valori della variabile l'espressione è negativa, 
per quali è nulla  e per quali è positiva.
Come esempio possiamo studiare i valori che assumono i due 
binomi~\(f(x) = -4 x +4\) e~\(g(x) = \dfrac{3}{2} x +3\) al variare di 
\(x\).

Per farlo costruiamo una tabella con alcuni valori e disegniamo il grafico 
delle rette: \quad \(y=f(x)\) e \(y=g(x)\).

\triaffiancati{.48}{.25}{.25}{
{\small
 \begin{tabular}{c|c|c}
...
\immagine[.8]{Retta decrescente la parte sopra l'asse x è colorata in blu, 
quella sotto in rosso.} 
{\disrettaa} 
\(\qquad y = -4 x +4\)
}{
\immagine[.8]{Retta crescente la parte sotto l'asse x è colorata in rosso, 
quella sopra in blu.} 
{\disrettab} 
\(\qquad y = \dfrac{3}{2} x +3\)
}
...
\ind{polinomio positivo o negativo}
...
molto approssimato: sono due gli aspetti che dobbiamo riportare nel 
grafico:

\medskip
\begin{enumerate} [nosep]
\item lo zero del polinomio, cioè il punto in cui la retta interseca 
l'asse~\(x\);
\item l'andamento della retta: cioè se la retta è crescente o decrescente.
\end{enumerate}
\ind{zero di un polinomio} \ind{retta crescente o decrescente}
\medskip
Si capisce facilmente se la retta è crescente o decrescente guardando la 
sua equazione infatti le rette \emph{crescenti} hanno il coefficiente 
della~\(x\) \emph{positivo}, mentre le rette \emph{decrescenti} hanno il 
coefficiente della~\(x\) \emph{negativo}.
Quindi i grafici possono essere tracciati semplicemente in questo modo:

\affiancati{.49}{.49}{
\immagine{Retta decrescente.}{\rettadec{+1}}
}{
\immagine{Retta decrescente.}{\rettacre{-2}}
}
...
\affiancati{.49}{.49}{
\immagine{Retta decrescente con indicati i segni: + 0 -.}{\segnidec{+1}}
}{
\immagine{Retta crescente con indicati i segni: - 0 +.}{\segnicre{-2}}
}
...
\end{enumeratea}
\end{procedura}
\ind{segno di un binomio di 1° grado}
Riassumendo, lo studio del segno del binomio di primo 
grado:~\(f(x) = -4 x +4\), si riduce a svolgere questi due passi:

\begin{enumerate}
\item Equazione Associata: \quad \(-4 x +4 = 0 \ssRarrow x = +1\) \\[-8mm]
\item Funzione Associata: \quad \(y = -4 x +4 \ssrarrow\)
\raisebox{-5mm}{
\immagine*{Retta decrescente con indicati i segni: + 0 -.}{\segnidec{+1}}}
\end{enumerate}
...
\ind{segno di un prodotto}
Una volta esperti sullo studio del segno di un binomio di primo grado, possiamo
...
stando ben attenti di riportarli in \emph{ordine crescente} e intestiamo 
ogni \emph{spazio orizzontale} con l'indicazione del fattore di cui 
vogliamo riportare il segno. 
...
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{x + 2 = 0 \quad \Rightarrow \quad x=-2}
           {y = x + 2 \quad \rightarrow}
           {\segnicre{-2}}
}
...
\item Quindi il prodotto 
\(f(x) = f_1(x) \cdot f_2(x)=(x -4)(x +2)\) è:\\[.2em]
\begin{itemize} [noitemsep, left=0mm]
\affiancati{.48}{.48}{
...
\begin{itemize} [noitemsep, left=0mm]
\item Studio del segno del numeratore~\(N\):\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{-3x-4 = 0 \quad \Rightarrow \quad x = -\dfrac{4}{3}}
           {y = -3x-4 \quad \rightarrow}
           {\segnidec{-\dfrac{4}{3}}}
}
\item Studio del segno del denominatore~\(D\):\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{2x+7 = 0 \quad \Rightarrow \quad x=-\dfrac{7}{2}}
           {y = 2x+7 \quad \rightarrow}
           {\segnicre{-\dfrac{7}{2}}}
}
\item Grafo dei segni \(\tonda{-\frac{7}{2} < -\frac{4}{3}\quad 
\text{ dato che} \quad -7 \cdot 3 < -4 \cdot 2}\):

\immagine{}{\segnoquozientea}

Oppure, usando il nome dato ai fattori: 

\immagine{}{\segnoquozienteas}

\item Possiamo concludere che il quoziente 
\(f(x) = \dfrac{-3x-4}{2x+7}\) è:\\[.2em]
\begin{itemize} [noitemsep, left=0mm]
\affiancati{.48}{.48}{
...
\label{sec:dis_combinazione}

 \begin{esempio}{}{}
Applichiamo questo procedimento allo studio del segno della frazione:
\[f(x)=\frac{x(1 -2 x)(1 + 2 x)}{(x -2)(x +5)}\]

Chiamiamo:~\(n_1\),~\(n_2\)~e~\(n_3\) i fattori che si trovano al 
numeratore 
e:~\(d_1\)~e~\(d_2\) i fattori che si trovano al denominatore.

\begin{itemize} [noitemsep, left=0mm]
\item Studio del segno del numeratore~\(n_1\):\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{x = 0}
           {y = x \quad \rightarrow}
           {\segnicre{0}}
}
\item Studio del segno del numeratore~\(n_2\):\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{-2x +1 = 0 \quad \Rightarrow \quad x = +\dfrac{1}{2}}
           {y = -2x +1 \quad \rightarrow}
           {\segnidec{+\dfrac{1}{2}}}
}
\item Studio del segno del numeratore~\(n_3\):\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{2x +1 = 0 \quad \Rightarrow \quad x = -\dfrac{1}{2}}
           {y = x -2 \quad \rightarrow}
           {\segnicre{-\dfrac{1}{2}}}
}
\item Studio del segno del denominatore~\(d_1\):\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{x -2 = 0 \quad \Rightarrow \quad x = +2}
           {y = x -2 \quad \rightarrow}
           {\segnicre{+2}}
}
\item Studio del segno del denominatore~\(d_2\):\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{x +5 = 0 \quad \Rightarrow \quad x=-5}
           {y = x +5 \quad \rightarrow}
           {\segnicre{-5}}
}
...
Ora si può osservare che il primo principio è semplice e esattamente 
uguale a quello delle equazioni, il secondo invece è insidioso\dots 
...
\ind{risolvere disequazioni lineari}
...
la disequazione trasformata in modo da avere il secondo membro uguale a 
zero.
\ind{disequazione in forma canonica}
...
La disequazione di diventa:~\(-2 x +6 > 0\) e il primo principio ci 
assicura che le soluzioni di questa disequazione sono tutte e sole le 
soluzioni della disequazione di partenza.
\item Studio del segno del polinomio: \\
\item Studio del segno del secondo fattore~\(f_2\):\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{-2 x +6 = 0 \Rightarrow  x=3}
           {y = -2x +6 \rightarrow }
           {\segnidec{+3}}
}
...
\subitem 
rappresentazione grafica: \quad
\raisebox{-3mm}{\disegno{\raylconasse{0}{5}{2.5}{+3}{white}{x}}}
\subitem rappresentazione con i predicati: \quad \(x < 3\) 
\subitem rappresentazione con le parentesi: \quad 
\(\intervaa{-\infty}{+3}\). 
\end{itemize}
\end{esempio}
...
l'asse~\(x\), cioè è parallela all'asse~\(x\). 
In questo caso tutti i valori della funzione staranno dalla stessa parte 
dell'asse cioè saranno \emph{tutti positivi} o \emph{tutti negativi}. 
...
La forma canonica è~\(0 x + 2 > 0\) che si riduce alla disuguaglianza 
\(0>-2\), che è vera per qualunque~\(x\) reale:~\(\IS=\R\).
\begin{itemize} [noitemsep, left=0mm]
 \item Studio del segno del polinomio:\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{0 x + 2 = 0 \Rightarrow  Impossibile}
           {y=0 x + 2 \rightarrow }
           {\segnipos}
}
\item Quindi per ogni valore di~\(x\) il polinomio è sempre positivo. 
\subitem rappresentazione grafica: 
\raisebox{-3mm}{\disegno{\allx{0}{6}{x}}}
...
\begin{itemize} [noitemsep]
 \item Studio del segno del polinomio:\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{0 x +1 = 0 \Rightarrow  Impossibile}
           {y=0 x +1 \rightarrow }
           {\segnipos}
}
\item Quindi per ogni valore di~\(x\) il polinomio è sempre positivo. 
...
quindi\dots
\subitem rappresentazione grafica: 
\raisebox{-3mm}{\disegno{\emptyx{0}{6}{x}}}
...
\ind{segno di un prodotto} \ind{segno di un quoziente}
Se una disequazione è formata dal prodotto o dal quoziente di polinomi, 
una volta scritta in forma normale,
dobbiamo studiare il segno di tutto il prodotto o il quoziente e poi 
risolvere la disequazione.
Vediamo un esempio.

 \begin{esempio}{}{}
Consideriamo:~\(\frac{(x-2)(-2x+1)}{x+3} \geqslant 0\).

Dobbiamo studiare il segno di ogni singolo fattore:
\begin{itemize} [noitemsep, left=0mm]
\item segno di \((x-2)\)\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{x -2 = 0 \Rightarrow x = +2}
           {y = x -2 \rightarrow }
           {\segnicre{+2}}
}
\item segno di \((-2x+1)\)\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{-2x +1 = 0 \Rightarrow  x = +\dfrac{1}{2}}
           {y=-2x +1 \rightarrow }
           {\segnidec{+\dfrac{1}{2}}}
}
 \item Segno del denominatore:\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{x +3 = 0 \Rightarrow  x = -3}
           {y = x +3 \rightarrow }
           {\segnicre{-3}}
}
...
valori che rendono \(f(x)\) non negativo, sono quelli 
che si trovano a sinistra di \(-3\) oppure che si trovano 
tra \(\dfrac{1}{2}\) e \(+2\), estremi compresi.
\subitem 
rappresentazione grafica: \quad \raisebox{-3mm}
...
predicati: \quad \(x < -3 \quad \vee \quad 
                +\dfrac{1}{2} \leqslant x \leqslant +2\) 
...
\ind{risolvere disequazioni fratte}
...
\begin{enumeratea}
\item spostare tutti i termini a primo membro e sommarli in modo da 
ottenere una sola frazione e lasciare a secondo membro solo lo zero;
...
\begin{esempio}{}{}
Consideriamo:~\(\dfrac{-3 x +4}{x+2} \leqslant-1\).
\begin{itemize} [noitemsep, left=0mm]
\item scrivere la disequazione in forma normale:\\ [.5em]
\(\dfrac{-3 x +4}{x+2} \leqslant -1 \sRarrow 
\dfrac{-3 x +4}{x+2} +1 \leqslant 0 \sRarrow
\dfrac{-3 x +4 + x +2}{x+2} \leqslant 0 \sRarrow \)
\(\dfrac{-2 x +6}{x+2} \leqslant0\)\\
\item Segno del numeratore:\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{-2x +6 = 0 \Rightarrow  +3}
           {y = -2x +6 \rightarrow }
           {\segnidec{+3}}
}
\item Segno del denominatore:\\
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{x +2 = 0 \Rightarrow  -2}
           {y = x +2 \rightarrow }
           {\segnicre{-2}}
}
...
\subitem rappresentazione grafica: \raisebox{-3mm}{
\disegno{\raylconasse{0}{6}{2}{-2}{white}{x}
      \ray{0}{6}{4}{+3}{blue}}
}
\subitem rappresentazione con i predicati: \quad \(x < -2 \sor x 
\geqslant +3\) 
\subitem rappresentazione con le parentesi: \quad 
\(\intervaa{-\infty}{-2} \quad \cup \quad \intervca{+3}{+\infty}\) 
\end{itemize}
\end{esempio}
...
\immagine[.8]{Soluzione di una disequazione fratta in un unico grafo.}
{\soluzionefratta}
\end{osservazione}
...
l'insieme dei numeri che sono soluzioni comuni alle disequazioni del 
sistema.
...
\ind{intersezione delle soluzioni}

Iniziamo con un esempio semplice:
\ind{risolvere un sistema di disequazioni}
...
la condizione~\(2x -1 \leqslant\dfrac{1}{2}x +2\)
...
\[\sistema{x > 0\\ 2x -1 \leqslant\dfrac{1}{2}x +2} \quad \sRarrow
  \sistema{x > 0\\ 3x -6 \leqslant0}\]
% \begin{minipage}{.65\textwidth}
La prima disequazione è banale, l'insieme soluzione \(IS_1\) è: \quad 
\raisebox{-3mm}{\immagine*{Asse con evidenziata la semiretta da 0 in 
poi.}{\sisdisa}}

Nella seconda, la soluzione dell'E.A. è~\(+2\); 
il grafico della F.A. è una retta crescente quindi:
\(IS_2\) è l'insieme di numeri minori o uguali a~\(+2\): \quad
\raisebox{-3mm}{\immagine*{Asse con evidenziata la semiretta fino a 
2.}{\sisdisb}}
...
\immagine{Grafico di 2 assi con evidenziati gli insiemi S1 e S2 e un 
terzo asse con l'intersezione di questi due intervalli.}
{\sistemaa}
...
\(\sistema{2 (x -5) \leqslant 3 + 4 x \\
6 x -4 < -3 x -2}\)
\begin{itemize} [left=0mm]%[noitemsep]
\item Primo passo: scrivere il sistema in forma normale:

\(\sistema{2 x -10 -3 - 4 x \leqslant 0 \\
6 x -4 +3 x +2 < 0} \ssRarrow\)
\(\sistema{-2 x -13 \leqslant 0 & \quad \mbox{(1)} \\
  9 x -2 < 0 & \quad \mbox{(2)}}\)

\item Secondo passo: risolvere le singole disequazioni

\begin{enumerate} [left=0mm, label=({\arabic*})]
\item 
Soluzione della prima disequazione:~\(-2 x -13 \leqslant 0\)

\hspace*{-10mm} \scalebox{.9}{
\segnoretta{-2x -13 = 0 \Rightarrow -\dfrac{13}{2}}
           {y = -2x -13 \rightarrow }
           {\segnidec{-\dfrac{13}{2}}}
}
\vspace{-10mm}
\immagine{Asse con evidenziato il punto -13/2 e i punti seguenti.}
{\disegno{\rayrconasse{0}{6}{3}{-\frac{13}{2}}{blue}{x}}}

\item Soluzione della seconda disequazione:~\(9 x -2 < 0\)

\hspace*{-10mm} \scalebox{.9}{
\segnoretta{9 x -2 = 0 \Rightarrow +\dfrac{2}{9}}
           {y = 9 x -2 \rightarrow }
           {\segnicre{+\dfrac{2}{9}}}
}
\vspace{-10mm}
\immagine{Asse con evidenziati i punti fino a +2/9 escluso.}
{\disegno{\raylconasse{0}{6}{3}{+\frac{2}{9}}{white}{x}}}
\end{enumerate}

\item Terzo passo: intersezione degli intervalli precedenti:\\
\vspace{-8mm}\hspace*{-5mm}
\immagine*[1]{Grafo del sistema con le soluzioni scritte nei tre 
modi.}
{\sistemab}
\vspace{3mm}
\end{itemize}
...
\ind{risolvere disequazioni letterali}
...
\(k \left( x -1 \right )\leqslant k \left ( k - x \right ) + x\)

Innanzitutto la scriviamo in forma normale:
\[k x - k \leqslant k^2 - k x + x \sRarrow k x - k - k^2 + k x - x \leqslant0 
\sRarrow
2 k x - x - k^2 - k \leqslant 0\]
Raccogliendo il fattore \(x\): \quad 
\(\left (2 k - 1 \right ) x - k^2 - k \leqslant 0\)
...
\hspace*{-5mm} \scalebox{.9}{
\segnoretta{2k -1 = 0 \Rightarrow k = +\dfrac{1}{2}}
           {y = 2k -1 \rightarrow }
           {\segnicre{+\dfrac{1}{2}}}
}
...
\vspace{-.5em} \hspace*{-5mm} \scalebox{.9}{
\segnoretta{x = \dfrac{k^2 - k}{2 k - 1}}
           {y = \dfrac{k^2 - k}{2 k - 1} \rightarrow }
           {\segnidec{\dfrac{k^2 - k}{2 k - 1}}}
}

\vspace{-2em}
\begin{itemize} [nosep]
\item 
Quindi se \quad \(k < \dfrac{1}{2}\) \qquad 
\raisebox{-9pt}{
\disegno{\rayrconasse{0}{6}{3}{\frac{k^2 - k}{2 k - 1}}{blue}{x}}
}
\qquad \(x \leqslant \dfrac{k^2 - k}{2 k - 1}\)
\end{itemize}

\item Se~\(k = \dfrac{1}{2} \Rightarrow 2 k - 1 = 0\)
coefficiente angolare zero, retta orizzontale:

\vspace{-.5em} \hspace*{-5mm} \scalebox{.9}{
\segnoretta{f(x) = 0 \Rightarrow Impossibile}
           {y = f(x) \rightarrow }
           {\segnineg}
}
...
\vspace{-.5em} \hspace*{-5mm} \scalebox{.9}{
\segnoretta{f(x) = 0 \Rightarrow x=\dfrac{k^2 - k}{2 k - 1}}
           {y = f(x) \rightarrow }
...
\begin{itemize} [noitemsep]
\item 
Se \(k < \dfrac{1}{2} \rightarrow\)\\
\(\rightarrow x \geqslant \dfrac{k^2 - k}{2 k - 1}\)
\item Se \(k = \dfrac{1}{2} \rightarrow\)\\[3mm]
\(\rightarrow \forall x \in \mathbb{R}\)
\item 
Se \(k > \dfrac{1}{2} \rightarrow\)\\
\(\rightarrow x \leqslant\dfrac{k^2 - k}{2 k - 1}\)
\end{itemize}
\end{htmulticols}
...
% spostando questi termini cambiano segno e otteniamo~\(-4>2x\). Il 
% coefficiente
% dell'incognita è positivo dunque applichiamo il
% secondo principio dividendo per~2,
% abbiamo~\(\frac{-4}{2}>\frac{2}{2}x\Rightarrow -2>x\), che letta da 
% destra a 
% sinistra dice che i
% valori da attribuire ad~\(x\) per soddisfare la disequazione assegnata 
% sono
...
\ind{problemi risolvibili con disequazioni}
...
% \[\IS=\{x\in \R/0<x\le~2\}=(0,2].\]
% 
% \end{soluzione}
% 
% \begin{problema}{}{}
%  In un triangolo il lato maggiore misura~\(13\munit{m}\), gli altri due 
% lati
% differiscono tra di loro di~\(2\munit{m}\). Come si deve scegliere il lato 
% minore
% affinché il perimetro non superi i~\(100\munit{m}\)?
% \end{problema}
% 
% \emph{Dati}:~\(\overline{AB}=13\munit{m}\), 
% \(\overline{BC}-\overline{AC}=2\munit{m}\).
% Riferendoci alla figura, \(AC\) è il lato minore; indichiamo con~\(x\) la 
% sua
% misura.
% \begin{center}
...
% % \end{center}
% % Affinché il perimetro non superi~\(100\munit{m}\) la misura in metri del
% % lato minore deve essere un numero dell'insieme:
% % \[\IS=\left\{x\in \R/\frac{11}{2}<x\le\frac{85}{2}\right\}.\]
% \end{soluzione}
...
% x>\frac{27}{10}\rightarrow\IS_{1}=\left\{x\in\R/x>\frac{27}{10}\right\},\]
% \[d_{2}:9x+9>15x-75-10x\Rightarrow~4x>-84\Rightarrow x>-21\rightarrow 
% \IS_{2}=\left\{x\in\R/x>-21\right\}.\]
% 
...
% \end{center}
% \[\IS=\left\{x\in\R/x>\frac{27}{10}\right\}.\]
%  \end{esempio}
% 
%  \begin{esempio}{}{}
%  Risolvere il seguente sistema di disequazioni.
...
% x\le\frac{3}{4}\rightarrow\IS_{2}=\left\{x\in \R/x\le\frac{3}{4}\right\}.\]
% 
...
%   
% 
% \dfrac{1}{3}\cdot\left(x-\dfrac{1}{2}\right)-
% \dfrac{1}{2}\cdot\left(x-\dfrac{1}{
...
% -{\frac{13}{10}}\rightarrow\IS_{2}=\left\{x\in\R/x\le 
% -{\frac{13}{10}}\right\}.\]
% 
...
% \ovalbox{\ref{ese:21.33}, \ref{ese:21.34}, \ref{ese:21.34}, 
% \ref{ese:21.35}, 
...
% \begin{problema}{}{}
% \label{pro:22.1}
% Determinare i valori di~\(x\) che rendono il polinomio~\(p=(3x-7)(2-x)\) 
% positivo.
% \end{problema}
% 
...
% \t	 x>2
% \t	 \end{array}\right.
% \rightarrow\IS_{2}=\left\{x\in\R/2<x<\dfrac{7}{3}\right\}.\]
% 
% Quindi~\(\IS=\IS_{1}\cup\IS_{2}=\left\{x\in\R/2<x<\dfrac{7}{3}\right\}\).
% 
...
% 
% \begin{itemize*}
% \item la semiretta in grassetto rappresenta i valori che rendono il 
% polinomio 
% positivo;
...
% concludere~\(\IS=\left\{x\in\R/2<x<\frac{7}{3}\right\}\).
%  \end{esempio}
% 
%  \begin{esempio}{}{}
...
% segno ``\(+\)''.
% \[\IS=\left\{x\in\R/x<\frac{4}{5}\vee~3<x<\frac{9}{2}\right\}.\]
% \end{esempio}
% 
% \begin{esempio}{}{}
%  \(4x^{3}+4x^{2}\le~1+x.\)
...
% \end{center}
% \[\IS=\left\{x\in \R/x\le-1\text{ oppure }-\frac{1}{2}\le 
% x\le\frac{1}{2}\right\}.\]
% \end{esempio}
% % \end{exrig}
% 
...
% \end{center}
% Analogamente a quanto fatto per il prodotto, dalla tabella dei segni 
% otteniamo
% \[\IS=\left\{x\in \R/2<x\le \frac{7}{3}\right\}\]
...
% membro;
%  \item eseguire i calcoli dell'espressione al primo membro per arrivare a 
% una 
...
%  \item studiare il segno del numeratore e del denominatore, 
% ponendo~\(N(x)>0\) 
% (oppure~\(N(x)\geq~0\) a secondo della richiesta) e~\(D(x)>0\);
%  \item costruire la tabella dei segni, segnando con un punto in grassetto 
% gli 
% zeri del numeratore;
%  \item determinare gli intervalli in cui il polinomio assume il segno 
% richiesto.
...
% \dfrac{4x^{2}(2x+1)+1}{8x^{3}+8x^{2
% }-2x-2}>0\).
% 
% Scomponiamo in fattori i denominatori, determiniamo il minimo comune
% multiplo e sommiamo le frazioni per arrivare alla 
% forma~\(\frac{N(x)}{D(x)}>0\):
% 
% \begin{align}
% 
...
% \Rightarrow & 
% 
% \frac{(x-1)(2x-1)(2x+1)+(2x+1)(2x+1)(x+1)-4x^{2}(2x+1)+1}{2(x+1)(2x-1)
% (2x+1)}>0 
% \notag\\
...
%  \[\IS=\left\{x\in \R/x<-1\vee -{\frac{1}{2}}<x<-{\frac{1}{4}}\vee 
% x>\frac{1}{2}\right\}.\]
% \end{esempio}
% 
%  \begin{esempio}{}{}
...
%  \[\begin{gathered}N\ge~0\Rightarrow~11x-2\ge~0\Rightarrow 
% x\ge\frac{2}{11},\\
% \t	  D>0\Rightarrow\left\{\begin{array}{l}
...
% \[\IS=\left\{x\in \R/x\le\frac{2}{11}\vee \frac{2}{3}<x<1\right\}.\]
%  \end{esempio}
% % \end{exrig}
